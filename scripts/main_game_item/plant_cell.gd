...
func create_plant(plant_type:CharacterRegistry.PlantType, is_imitater:=false, is_plant_start_effect:=true, is_imitater_material:=false, is_zombie_mode:=false) -> Plant000Base:
	...
	## 融合判断：若目标位置已有植物，且和当前种下的植物存在融合配方，则替换为融合结果
	var is_fusion := false
	if not is_imitater:
		var existing_plant:Plant000Base = plant_in_cell[plant_condition.place_plant_in_cell]
		if is_instance_valid(existing_plant):
			var fusion_result:CharacterRegistry.PlantType = Global.character_registry.get_fusion_result(existing_plant.plant_type, plant_type)
			if fusion_result != CharacterRegistry.PlantType.Null:
				## 删除原有植物，改为种植融合后的植物
				existing_plant.character_death_disappear()
				## queue_free()是延迟释放的，本帧内is_instance_valid仍会返回true，
				## 所以要立刻清空字典引用，避免下面的占用检测误判为已被占用
				plant_in_cell[plant_condition.place_plant_in_cell] = null
				plant_type = fusion_result
				plant_condition = Global.character_registry.get_plant_info(plant_type, CharacterRegistry.PlantInfoAttribute.PlantConditionResource)
				is_fusion = true
	...
	if plant_condition.is_purple_card:
		...
		## 非紫卡 如果该位置已经存在植物,返回（融合已经处理过占用情况，跳过检测）
		if not is_fusion and is_instance_valid(plant_in_cell[plant_condition.place_plant_in_cell]):
