...
@onready var hm_glove: HM_Glove = $HM_Glove
...
func _process(_delta: float) -> void:
	match curr_hm_status:
		...
		E_HandManagerStatus.Glove:
			hm_glove.item_process()
...
func _click_glove() -> void:
	SoundManager.play_other_SFX("shovel")
	curr_hm_status = E_HandManagerStatus.Glove
	hm_glove.click_glove()
	## 如果当前在植物格子中
	if curr_plant_cell:
		_on_cell_mouse_enter(curr_plant_cell)
...
func _on_click_cell(plant_cell:PlantCell):
	match curr_hm_status:
		...
		E_HandManagerStatus.Item:
			...
			curr_hm_status = E_HandManagerStatus.Null
		E_HandManagerStatus.Glove:
			hm_glove.click_cell(plant_cell)
			## Huwag agad ibalik sa Null kung kakapick-up lang (unang click),
			## dapat manatiling Glove state hanggang malagay yung plant sa 2nd click
			if hm_glove.picked_plant_type == CharacterRegistry.PlantType.Null:
				curr_hm_status = E_HandManagerStatus.Null
...
func _on_cell_mouse_enter(plant_cell:PlantCell):
	...
	match curr_hm_status:
		...
		E_HandManagerStatus.Glove:
			hm_glove.mouse_enter(plant_cell)
...
func _on_cell_mouse_exit(plant_cell:PlantCell):
	...
	match curr_hm_status:
		...
		E_HandManagerStatus.Glove:
			hm_glove.mouse_exit(plant_cell)
