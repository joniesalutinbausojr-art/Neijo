...
const zombie_power = {
	...
	CharacterRegistry.ZombieType.Z026RedEyeGargantuar: 10,	# 红眼伽刚特尔 (bago)
...
const zombie_weights_ori = {
	...
	CharacterRegistry.ZombieType.Z026RedEyeGargantuar: 500,	# 红眼伽刚特尔 (bago — mas mababa/mas rare)
...
func spawn_bungi_zombies():
	...
	for plant_cell:PlantCell in all_cell_be_bungi:
		...
		zombie_manager.create_norm_zombie(
			...
			GlobalUtils.create_bookbungi.bind(plant_cell)
		)
