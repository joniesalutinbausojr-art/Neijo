...
func judge_is_can_plant(plant_cell:PlantCell, curr_plant_type:CharacterRegistry.PlantType) -> bool:
	...
	## 普通植物非紫卡
		...
		## 地形不符合，或者当前不能种植普通植物，直接不可种植
		if not (plant_cell.can_common_plant and plant_condition & plant_cell.curr_condition):
			return false

		var existing_plant:Plant000Base = plant_cell.plant_in_cell[place_plant_in_cell]
		## 当前格子对应的植物位置为空，走正常种植判断
		if not is_instance_valid(existing_plant):
			...
			return true
		## 位置已有植物：如果存在融合配方，也允许种植（会触发融合）
		elif Global.character_registry.get_fusion_result(existing_plant.plant_type, curr_plant_type) != CharacterRegistry.PlantType.Null:
			return true
		else:
			return false
	else:
